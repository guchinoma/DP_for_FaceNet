...
# Below is the the added functions for LFW model.
# They are linked by the lfw_driver.py
################################################################################

def read_lfw(config, filename_queue):
    """Reads and parses examples from CIFAR10 data files.
    ...
    # Read a record, getting filenames from the filename_queue.  No
    # header or footer in the LFW format, so we leave header_bytes
    # and footer_bytes at their default of 0.
    reader = tf.FixedLengthRecordReader(record_bytes=record_bytes)
    result.key, value = reader.read(filename_queue)
...
def lfw_inputs(config, distort=False, whiten=True, for_eval=False, shuffle=False):
    """Construct input for LFW evaluation using the Reader ops.
    ...
    data_dir = os.path.join(config.get('data', 'work_directory'), '')
    batch_size = int(config.get('main', 'batch_size'))
...
def read_lfw(image_list, label_list):
